...
from utils.dataset import dataset as datasets
...
def run_model(nb_kpts, net_pred, optimizer=None, data_loader=None, opt=None, is_train=0, epo=0):
    ...
    if is_train == 1: #train
        ...
        for i, (data_) in enumerate(data_loader): # raw_in_n + out_n
            ...
            if opt.vis:
                save_path = 'outputs/'+'{}'.format(i)+'/'
                vis_2p.vis_pi_compare(data_.reshape([-1, seq_len, nb_kpts,3]).detach().cpu().numpy()[13],pred.detach().cpu().numpy()[13], save_path)
        ...
        if opt.save_results:
            ...
            file_path = "outputs/results.json"
...
if __name__ == '__main__':
    ...
    if num_gpus >0:
        print(f"Found {num_gpus} CUDA device(s) available:")
        for gpu_id in range(num_gpus):
            gpu_name = torch.cuda.get_device_name(gpu_id)
            print(f"GPU {gpu_id}: {gpu_name}")
    else:
        print("No CUDA devices found on this system.")
