...
class AttModel(Module):
    ...
    def __init__(self, in_features=37*2*3, kernel_size=10, d_model=256, num_stage=12, dct_n=20, input_n=50):
        ...
        self.update_v1 = XIA.XIA(embed_dim=self.in_features, nb_h=3, dropout=0.1) # in_features = 111
        self.update_v2 = XIA.XIA(embed_dim=self.in_features, nb_h=3, dropout=0.1)
...
if __name__ == '__main__':
    p3d_src = torch.rand((32,75,37*2*3)).cuda()
    net_pred = AttModel(in_features=37*2*3,
            kernel_size=10,
            d_model=256,
            num_stage=12,
            dct_n=20).cuda().eval()
