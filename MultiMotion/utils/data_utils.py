...
def normExPI_2p_by_frame(seq1, seq2):
    nb, dim = seq1.shape # nb_frames, dim=37*3+1
    seq_norm = np.empty((nb,(dim-1)*2))
    for i in range(nb):
        img1 = seq1[i][1:].reshape((-1,3)) #37
        img2 = seq2[i][1:].reshape((-1, 3))#37

        temp =np.append(img1, img2)
        temp = temp.reshape((-1, 3))  # 37

        P0 = img1[0] #hip
        P1 = img1[7] #rightupleg
        P2 = img1[33] #neck
        img_norm = normExPI_xoz(temp, P0,P1,P2)
        seq_norm[i] = img_norm.reshape((dim-1)*2)
    return seq_norm
